...
{
    if (r < 0 || r >= maze.size() || c < 0 || c >= maze[0].size()) {  //checks if current cell hits a wall or goes out of bounds
        return false;
    }

    if (maze[r][c] == 1) {                                           //checks if curent cell is wall
        return false;
    }

    if (visited[r][c]) {                                             //checks if cell has been visited
        return false;
    }

    visited[r][c] = true;                                            //sets cell as visited

    if (r == exit_r && c == exit_c) {                                //checks if current cell is exit
        return true;
    }

    for (int i = 0; i < 4; i++) {                                   //checks up down left and right directions
        int nr = r + dr[i];
        int nc = c + dc[i];

        if (nr >= 0 && nr < maze.size() && nc >= 0 && nc < maze[0].size()) {                           //checks if next cell is in bounds
            if (!visited[nr][nc] && maze[nr][nc] == 0) {                                               //checks if next cell is open and hasn't been visited
                parent_r[nr][nc] = r;
                parent_c[nr][nc] = c;

                if (dfs(nr, nc, maze, visited, parent_r, parent_c, exit_r, exit_c)) {     //uses recursion to check next cells
                    return true;
                }
            }
        }
